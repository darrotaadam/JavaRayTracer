...
public class Main {
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<sceneDirs.length; i++) {
			...
			List<String> sceneFiles = listSceneFiles(new File(SCENES+sceneDirs[i]));
			for(int j=0; j<sceneFiles.size(); j++) {
				System.out.println("[*] Import de " + sceneFiles.get(j));
				
				try {
					scene = new Scene(SCENES + sceneDirs[i] + "/" + sceneFiles.get(j));
					renderer = new Renderer(scene);
					resultImage = renderer.render();
					resultImageName = resultImagesDir + sceneDirs[i] + "/" + sceneFiles.get(j).substring(0,sceneFiles.get(j).indexOf(".")) +".png";
